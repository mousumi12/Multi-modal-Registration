...
class CNN_scale1(object):
  ...
  def __call__(self, x):
    with tf.variable_scope(self.name, reuse=self.reuse):
      ...
      x_6 = conv2d_transpose(x_5, 'deconv1', 128, [5, 16, 16, 128], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
      x_7 = conv2d(x_6, 'Conv4', 64, 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
      x_8 = conv2d_transpose(x_7, 'deconv2', 64, [5, 16, 16, 64], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
...
class CNN_scale2(object):
	...
	def __call__(self, x):
		with tf.variable_scope(self.name, reuse=self.reuse):
			...
			x_4 = tf.nn.avg_pool(x_3, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME', name='pooling2')
			x_5 = conv2d(x_4, 'Conv3', 128, 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_6 = conv2d_transpose(x_5, 'deconv1', 128, [5, 32, 32, 128], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_7 = conv2d(x_6, 'Conv4', 64, 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_8 = conv2d_transpose(x_7, 'deconv2', 64, [5, 32, 32, 64], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_9 = reg(x_8, 'Reg1', 2, 3, 1, 'SAME', self.is_train)
			x_10 = reg(x_7, 'Reg2', 2, 3, 1, 'SAME', self.is_train)
			x_11 = reg(x_5, 'Reg3', 2, 3, 1, 'SAME', self.is_train)
...
class CNN_scale3(object):
	...
	def __call__(self, x):
		with tf.variable_scope(self.name, reuse=self.reuse):
			...
			x_4 = tf.nn.avg_pool(x_3, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME', name='pooling2')
			x_5 = conv2d(x_4, 'Conv3', 128, 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_6 = conv2d_transpose(x_5, 'deconv1', 64, [5, 64, 64, 64], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_7 = conv2d(x_6, 'Conv4', 64, 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_8 = conv2d_transpose(x_7, 'deconv2', 32, [5, 64, 64, 32], 3, 1, 'SAME', True, tf.nn.relu, self.is_train)
			x_9 = reg(x_8, 'Reg1', 2, 3, 1, 'SAME', self.is_train)
			x_10 = reg(x_7, 'Reg2', 2, 3, 1, 'SAME', self.is_train)
			x_11 = reg(x_5, 'Reg3', 2, 3, 1, 'SAME', self.is_train)
...
class DIRNet(object):
  def __init__(self, sess, config, name, is_train):
    ...
    #self.xy_down4 = bicubic_interp_2d(self.xy,[256/4, 256/4])
    ...
    ###########################
    ## Scale 2######
    #self.xy = tf.concat([self.x,self.y],3)  (#[self.z_scale1, self.y], 3)
    ...
    self.xy_down2 = bicubic_interp_2d(self.xy, [256/2, 256/2])
    ...
    self.v_down2 = self.vCNN2(self.xy_down2)  # residual dvf at scale 2
    ...
    self.z1  = WarpST(self.x, self.V, config.im_size)
    ...
    self.loss4 = gradientLoss('l2')(self.V)/490
    self.loss5 = gradientLoss('l2')(self.vres2)/90
    self.loss6 = gradientLoss('l2')(self.vres3)/90

    #self.loss = #(0.05*self.loss_scale1 +
    
    self.loss =	 (0.05*self.loss_scale2 + 0.95*self.loss_scale31 + self.loss4) # + 0.6 * (self.loss_scale32 +self.loss5) + 0.3 * (self.loss_scale33 + self.loss6)
    ...
    if self.is_train :
      ...
      self.train = self.optim.minimize(self.loss, var_list=[self.vCNN2.var_list,self.vCNN3.var_list], global_step=global_step)
  ...
  def deploy(self, dir_path, x, y, name1):
    ...
    return loss, loss_ncc, loss_ssim, loss_mse
